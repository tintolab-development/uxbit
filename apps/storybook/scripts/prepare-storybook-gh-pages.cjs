...
const STENCIL_ESM_DIR = path.resolve(
    __dirname,
    '..',
    '..',
    '..',
...
async function ensureFallbackFiles() {
    try {
        ...
        throw new Error(
            `Cannot find ${SOURCE_FILE}. Run "pnpm --filter storybook build" first.`,
        );
    }
...
async function copyStencilBundles() {
    try {
        ...
        throw new Error(
            `Cannot find ${STENCIL_ESM_DIR}. Run "pnpm --filter @uxbit/stencil-components build" before Storybook build.`,
        );
    }
