...
const TintoTabs = class {
  ...
  /* ============================ Render ============================ */
  render() {
    // render 시점에 탭 패널 수집 (Shadow DOM 사용 시 slot 내부 요소 접근)
    // componentDidLoad 이후에는 this.tabPanels 사용, 초기 렌더링 시에는 직접 수집
    const panels =
      this.tabPanels.length > 0
        ? this.tabPanels
        : Array.from(this.el.querySelectorAll('tinto-tab-panel'));
    // 기본 활성 탭 설정 (아직 설정되지 않은 경우)
    if (!this.activeTabId && panels.length > 0) {
      const defaultPanel = this.defaultTab
        ? panels.find((p) => p.tabId === this.defaultTab)
        : panels[0];
      if (defaultPanel && !defaultPanel.disabled) {
        this.activeTabId = defaultPanel.tabId;
      }
    ...
    return h(
      'div',
      { key: '37857250dc4eaef9d081747785fc7d55214d8588', class: 'tabs-container', part: 'root' },
      h(
        'div',
        {
          key: '01a0567059a7eca7b74747575d5fd8ae77d7396c',
          class: tabListClass,
          part: 'list',
          role: 'tablist',
      ...
      h(
        'div',
        { key: '0792b8474c04ed1df8878b45a713e7ee9a08b500', class: 'tabs-content', part: 'content' },
        h('slot', { key: '93ef4854b9b43d2db361bd73830d927a09606dc8' }),
      ),
    );
  }
};
