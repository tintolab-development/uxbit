...
| Prop       | 타입                                   | 기본값      | 설명           |
| ---------- | -------------------------------------- | ----------- | -------------- |
| `variant`  | `'default' \| 'minimal' \| 'bordered'` | `'default'` | 캐러셀 variant |
| `size`     | `'sm' \| 'md' \| 'lg'`                 | `'md'`      | 캐러셀 크기    |
...
```html
<!-- Minimal variant -->
<tinto-carousel variant="minimal" size="sm">
  <!-- 슬라이드 -->
</tinto-carousel>

<!-- Bordered variant, Large size -->
<tinto-carousel variant="bordered" size="lg">
  <!-- 슬라이드 -->
</tinto-carousel>

<!-- Disabled 상태 -->
<tinto-carousel disabled>
  <!-- 슬라이드 -->
</tinto-carousel>
```
...
- ✅ ARIA 속성 (`role="region"`, `aria-label`, `aria-live="polite"`)
- ✅ 인디케이터에 `role="tablist"`, `aria-selected` 지원
- ✅ 네비게이션 버튼에 `aria-disabled` 지원
- ✅ 슬라이드 컨테이너에 `aria-label` 자동 생성
- ✅ 스크린 리더 지원
- ✅ 포커스 관리
- ✅ `disabled` 상태에서 모든 상호작용 차단
