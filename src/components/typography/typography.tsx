...
import { Align, Color, FontFamily, FontSize, Variant, HighlightColor, FAMILY_MAP, FONT_SIZE, TypingUnit } from './typography.types';
...
export class TintoTypography {
  ...
  render() {
    ...
    const content = this.href ? (
      <a part="link" href={this.href} target={this.target} rel={rel} style={{ color: 'inherit', textDecoration: 'inherit' }}>
        {typedSpan}
      </a>
    ) : (
      typedSpan
    ...
    return (
      <Tag part="root" class={`tinto-typography ${this.variant} ${this.rolling ? 'has-typing' : ''}`} style={style} aria-hidden={ariaHidden as any}>
        {content}
      </Tag>
    );
  }
